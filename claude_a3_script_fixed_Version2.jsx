#targetengine "claudeA3Engine"

/*
 * ClaudeA3.jsx (fixed for InDesign 19 modeless/session issues)
 * Məqsəd: Qəzet və jurnal üçün məzmunları (mətn, şəkil) avtomatik olaraq 
 * InDesign səhifələrinə strukturlu şəkildə yerləşdirmək
 * Versiya: 3.1 -> 3.1-fix (Session/Palette tweak)
 * Tarix: 2025 (patched 2026)
 */

// Əvvəlki persistent pəncərə varsa bağla (əgər skripti təkrar işə salmısan)
try {
    if ($.global.claudeA3Win && $.global.claudeA3Win instanceof Window) {
        try { $.global.claudeA3Win.close(); } catch(e) {}
        $.global.claudeA3Win = null;
    }
} catch(e) { /* ignore */ }
...
// ============================================================================
// 3. GUI İNTERFEYS (modeless palette - InDesign19 üçün daha etibarlı)
// ============================================================================

// GUI yarat (palette modeless pəncərə - event delivery 19.0-da problemsiz işləməsi üçün)
var win;
try {
    win = new Window("palette", "ClaudeA3 - Qəzet Məzmun Yerləşdirici v3.1");
} catch(e) {
    // Fallback: əgər palette də problem yaratsa, dialog istifadə et
    try {
        win = new Window("dialog", "ClaudeA3 - Qəzet Məzmun Yerləşdirici v3.1");
    } catch(e2) {
        alert("Pəncərə yaradıla bilmədi: " + e2.toString());
        throw e2;
    }
}
...
try { txtProgress.graphics.font = "dialog:12"; } catch(e){}
...
btnRun.onClick = function() {
    ...
    // UI dialoqlarını gizlətməyi başlat (sonra finally-da qaytarılır)
    try {
        app.scriptPreferences.userInteractionLevel = UserInteractionLevels.NEVER_INTERACT;
    } catch(e) {
        // bəzi mühitlərdə bunun icazəsi yoxdur; davam et
    }
    ...
    try {
        ...
        // modeless pəncərəni bağlamaq istəmirsinizsə bu sətiri şərhə alın
        // win.close();
        
    } catch (e) {
        log("\n❌ KRİTİK XƏTA: " + e.toString());
        try { log("    Sətir: " + e.line); } catch(_) {}
        txtLog.text = LOG.join("\n");
...
// ============================================================================
// Pəncərə bağlanma təmizliyi və göstərilməsi
// ============================================================================

win.onClose = function() {
    try { app.scriptPreferences.userInteractionLevel = UserInteractionLevels.INTERACT_WITH_ALL; } catch(e) {}
    try { $.global.claudeA3Win = null; } catch(e) {}
};

$.global.claudeA3Win = win;
