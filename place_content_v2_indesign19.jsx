...
var win = new Window("palette", "Qəzet Məzmun Yerləşdiricisi v3.0", undefined, {resizeable: true});
...
btnRun.onClick = function() {
    ...
    try {
        ...
        // ✅ FIX: Səhifə döngüsü BAŞLANGICI
        for (var pageIndex = 1; pageIndex <= 7; pageIndex++) {
            var pageNum = pageIndex + 1;
            
            // ✅ FIX: PAGE DƏYIŞƏNI HƏLƏ DÖNGÜNÜN BAŞLANĞICINDA TƏRİF EDİL
            var page = doc.pages[pageIndex];
            
            log("\n═══ SƏHİFƏ " + pageNum + " (index: " + pageIndex + ") ═══");
            
            if (!page || !page.isValid) {
                log("❌ Səhifə mövcud deyil");
                continue;
            }
            ...
            // ✅ FIX: TEMİZLƏMƏ - PAGE İNDİ TƏRİF EDİLİB
            log("Səhifə " + pageNum + " təmizlənir...");
            try {
                page.pageItems.everyItem().remove();
                log("  ✓ Elementlər silindi");
            } catch(e) {
                log("  ⚠️ Təmizləmə xətası: " + e.toString());
            }
