...
var CONFIG = {
    ...
    defaults: {
        columns: 3,              // Təbib qəzeti üçün 3 sütun (peşəkar standart)
        imageRatio: 40,
        padding: 5,
        titleSize: 14,
        bodySize: 10,
    ...
    typography: {
        // Peşəkar qəzet tipoqrafiyası
        firstLineIndent: "3mm",  // Abzas başlanğıcı daxilə çəkilməsi
        bodyLeadingRatio: 1.2,   // Sətirarası: font ölçüsünün 120%-i
        hyphenateMinLength: 5,   // Azərbaycan dili üçün 5 hərf
        titleSpaceAfter: 4.5,    // Başlıq-mətn arası
        baselineGrid: 12         // Baseline grid (pt)
    }
};
...
function ensureStyles(doc) {
    ...
    try {
        ...
        titleStyle.spaceAfter = CONFIG.typography.titleSpaceAfter; // 4.5pt - başlıq yaxın mətnə
        titleStyle.leading = 16;
        titleStyle.fillColor = doc.swatches.itemByName("Black");
        titleStyle.hyphenation = false;
        titleStyle.keepWithNext = true; // Başlıq öz mətnindən ayrılmasın
        ...
        // PEŞƏKAR QƏZET TİPOQRAFİYASI
        bodyStyle.firstLineIndent = CONFIG.typography.firstLineIndent; // 3mm daxilə
        bodyStyle.leading = bodyStyle.pointSize * CONFIG.typography.bodyLeadingRatio; // 10pt × 1.2 = 12pt
        bodyStyle.hyphenation = true;
        bodyStyle.fillColor = doc.swatches.itemByName("Black");
        
        // Baseline Grid alignment (İnDesign-da əlavə tənzimləmə tələb edir)
        try {
            bodyStyle.alignToBaseline = true;
            bodyStyle.gridAlignFirstLineOnly = false;
        } catch(e) {
            log("  ⚠️ Baseline grid dəstəklənmir: " + e.message);
        }
        
        // Hyphenation settings - Azərbaycan dilinə uyğunlaşdırılmış
        try {
            bodyStyle.hyphenateCapitalizedWords = false;
            bodyStyle.hyphenateWordsLongerThan = CONFIG.typography.hyphenateMinLength; // 5 hərf
            bodyStyle.hyphenateAfterFirst = 2;
            bodyStyle.hyphenateBeforeLast = 2;
            bodyStyle.hyphenationZone = "3mm";
            bodyStyle.hyphenWeight = 9; // Hecalamanı daha çox təşviq et
        } catch(e) {
            log("⚠️ Hyphenation ayarları xətası: " + e.message);
        }
        
        log("✓ Əsas mətn üslubu: " + CONFIG.styles.body + " (FirstLineIndent: " + CONFIG.typography.firstLineIndent + ")");
        
        // 3. Şəkil açıqlaması üslubu (Caption)
        ...
        captionStyle.justification = Justification.LEFT_ALIGN; // Sol hizalama (peşəkar standart)
        captionStyle.spaceBefore = 2;
        captionStyle.spaceAfter = 0;
        captionStyle.leading = 9.6; // 8pt × 1.2
        captionStyle.fillColor = doc.swatches.itemByName("Black");
        captionStyle.hyphenation = false; // Caption-larda hecalama yoxdur
        
        // Caption mətnin şəkildən ayrılması üçün
        try {
            captionStyle.firstLineIndent = 0;
            captionStyle.leftIndent = "1mm";
            captionStyle.rightIndent = "1mm";
        } catch(e) {}
...
// Baseline Grid setup (isteğe bağlı)
function setupBaselineGrid(doc) {
    try {
        var gridPrefs = doc.gridPreferences;
        gridPrefs.baselineStart = "12pt";
        gridPrefs.baselineDivision = CONFIG.typography.baselineGrid; // 12pt
        gridPrefs.baselineGridShown = false; // İstifadəçi özü açar
        
        log("✓ Baseline Grid: " + CONFIG.typography.baselineGrid + "pt");
        return true;
    } catch(e) {
        log("⚠️ Baseline Grid ayarı xətası: " + e.message);
        return false;
    }
}
...
ddlColumns.selection = CONFIG.defaults.columns - 1; // Default: 3 sütun
...
var txtStyleInfo = grpStyleInfo.add("statictext", undefined, 
    "Bu skript avtomatik olaraq peşəkar qəzet üslublarını yaradır:\n\n" +
    "• Qezet_Basliq — Başlıqlar (14pt, Bold, Sol, keepWithNext)\n" +
    "• Qezet_Metn — Əsas mətn (10pt, İki tərəfə, 3mm ilk sətir indent)\n" +
    "• Qezet_Sekil_Aciklama — Caption (8pt, Italic, Sol)\n\n" +
    "TİPOQRAFİK STANDARTLAR:\n" +
    "✓ Leading: Font ölçüsünün 120%-i (10pt → 12pt)\n" +
    "✓ Hyphenation: 5+ hərf (Azərbaycan dili)\n" +
    "✓ First Line Indent: 3mm (abzas daxilə çəkilməsi)\n" +
    "✓ Baseline Grid: 12pt (əlavə tənzimləmə tələb edir)\n\n" +
    "Üslubları Paragraph Styles panelindən redaktə edə bilərsiniz.",
    {multiline: true}
);
txtStyleInfo.preferredSize = [580, 180];
...
btnRun.onClick = function() {
    ...
    try {
        ...
        if (chkCreateStyles.value) {
            ...
            // Baseline Grid ayarla (peşəkar qəzet tərtibatı üçün)
