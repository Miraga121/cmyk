// ═══════════════════════════════════════════════════════════
//  QƏZET MƏZMUN YERLƏŞDİRİCİSİ v3.0 - InDesign 19.0+
// ═══════════════════════════════════════════════════════════
// Təkmilləşdirilmiş və optimallaşdırılmış versiya
...
// ═══════════════════════════════════════════════════════════
//  GUI - ENHANCED İSTİFADƏÇİ İNTERFEYSİ
// ═══════════════════════════════════════════════════════════
...
txtHeader.graphics.font = ScriptUI.newFont(txtHeader.graphics.font.name, "BOLD", 11);
...
// ═══════════════════════════════════════════════════════════
//  HELPER FUNKSIYALAR - TƏKMILLƏŞDIRILMIŞ
// ═══════════════════════════════════════════════════════════
...
function readTextFile(file) {
    ...
    try {
        ...
        log("Fayl oxuma xətası: " + file.name + " - " + e.message, "error");
    }
    return content;
}
...
btnSelectAll.onClick = function() {
    for (var i = 0; i < chkPages.length; i++) {
        chkPages[i].value = true;
    }
};

btnDeselectAll.onClick = function() {
    for (var i = 0; i < chkPages.length; i++) {
        chkPages[i].value = false;
    }
};
...
btnTest.onClick = function() {
    ...
    log("InDesign versiya: " + app.version);
    log("Sənəd: " + doc.name + " (" + doc.pages.length + " səhifə)");
    ...
    for (var pageNum = 2; pageNum <= 8; pageNum++) {
        ...
        if (!pageFolder.exists) {
            log("Qovluq yoxdur: page" + pageNum, "warning");
            continue;
        }
        ...
        // İlk bir neçə faylın adını göstər
        if (txtFiles.length > 0) {
            log("Məsələn: " + txtFiles[0].name + (txtFiles.length > 1 ? ", " + txtFiles[1].name : ""));
        }
    }
    
    log("\n═══════════════════════════════════════");
    log("TEST NƏTİCƏSİ:");
    log("Cəmi " + totalTxt + " mətn faylı");
    log("Cəmi " + totalImg + " şəkil faylı");
    log("═══════════════════════════════════════");
    
    alert("✅ Test tamamlandı!\n\n" +
          "📄 " + totalTxt + " mətn faylı\n" +
          "🖼️ " + totalImg + " şəkil faylı tapıldı\n\n" +
          "Ətraflı məlumat üçün konsola (Window > Utilities > ExtendScript Toolkit) baxın.");
    
    txtProgress.text = "Test: " + totalTxt + " mətn, " + totalImg + " şəkil tapıldı";
};

// ═══════════════════════════════════════════════════════════
//  ƏSAS YERLƏŞDIRMƏ FUNKSIYASI
// ═══════════════════════════════════════════════════════════
...
btnRun.onClick = function() {
    ...
    // Seçilmiş səhifələri yığ
    for (var i = 0; i < chkPages.length; i++) {
        if (chkPages[i].value) {
            params.selectedPages.push(i + 2);
        }
    }
    
    if (params.selectedPages.length === 0) {
        alert("⚠️ Zəhmət olmasa ən azı bir səhifə seçin!");
        return;
    }
    
    if (!params.rootPath || params.rootPath === "") {
        alert("⚠️ Zəhmət olmasa qovluq seçin!");
        return;
    }
...
function processLayout(params, progressText) {
    ...
    log("═══════════════════════════════════════");
    log("YERLƏŞDIRMƏ BAŞLADI");
    log("═══════════════════════════════════════");
    log("Parametrlər: " + params.columns + " sütun, " + Math.round(params.imageRatio*100) + "% şəkil");
    ...
    for (var i = 0; i < params.selectedPages.length; i++) {
        ...
        if (progressText && progressText.window) {
            try {
                ...
                // Progress update xətası - davam et
            }
        }
        ...
        if (!page.isValid) {
            log("Səhifə " + pageNum + " sənəddə yoxdur", "error");
            continue;
        }
        ...
        if (params.clearExisting) {
            try {
                ...
                log("Elementləri silməkdə xəta: " + e.message, "warning");
            }
        }
        ...
        if (params.createLayers) {
            ...
            try {
                ...
                if (!layer.isValid) {
                    ...
                    log("Layer yaradıldı: " + layerName, "success");
                }
            } catch (e) {
        ...
        var txtFiles = getNumberedFiles(pageFolder, /\.txt$/i);
        if (txtFiles.length === 0) {
            log("Mətn faylı tapılmadı", "warning");
            continue;
        }
        
        log("İşlənilir: " + txtFiles.length + " mətn faylı");
        
        // Səhifə ölçüləri və grid hesablaması
        var pageBounds = page.bounds;
        var margins = page.marginPreferences;
        
        var usableW = pageBounds[3] - pageBounds[1] - margins.left - margins.right;
        var usableH = pageBounds[2] - pageBounds[0] - margins.top - margins.bottom;
        var startX = pageBounds[1] + margins.left;
        var startY = pageBounds[0] + margins.top;
        
        var cellW = usableW / params.columns;
        var rows = Math.ceil(txtFiles.length / params.columns);
        var cellH = usableH / rows;
        
        log("Grid: " + params.columns + "x" + rows + " (hüceyrə: " + Math.round(cellW) + "x" + Math.round(cellH) + ")");
        
        // Hər mətn faylını yerləşdir
        for (var t = 0; t < txtFiles.length; t++) {
            ...
            log("\n  [" + (t + 1) + "/" + txtFiles.length + "] " + txtFile.name);
            
            // Mətn oxu
            var content = readTextFile(txtFile);
            if (!content || content.trim() === "") {
                log("    Mətn boşdur", "warning");
                continue;
            }
            ...
            if (cleanLines.length === 0) {
                log("    Mətn təmiz sətir yoxdur", "warning");
                continue;
            }
            ...
            if (params.alternateBg && t % 2 !== 0) {
                try {
                    ...
                    log("    Arxa fon xətası: " + e.message, "warning");
                }
            }
            
            // Şəkilləri yerləşdir
            if (imgFiles.length > 0) {
                log("    Şəkillər: " + imgFiles.length);
                
                var imgContainerHeight = cellH * params.imageRatio;
                var maxImgs = Math.min(imgFiles.length, 4);
                var singleImgHeight = (imgContainerHeight / Math.ceil(maxImgs / 2)) - params.padding;
                
                for (var im = 0; im < maxImgs; im++) {
                    var imgCol = im % 2;
                    var imgRow = Math.floor(im / 2);
                    var imgX = x + params.padding + (imgCol * (cellW / 2));
                    var imgY = currentY + params.padding + (imgRow * (singleImgHeight + params.padding));
                    ...
                    try {
                        ...
                        STATS.placed++;
                        log("      ✓ " + imgFiles[im].name, "success");
                    } catch (e) {
                        log("      ✗ " + imgFiles[im].name + ": " + e.message, "error");
                    }
                }
                ...
                try {
                    ...
                    if (params.titleBold) {
                        try {
                            ...
                            // Bold mövcud deyilsə, sadəcə normal olaraq saxla
                        }
                    }
            ...
            if (body && body !== "") {
                try {
                    ...
                    if (remainingHeight > params.bodySize * 2) {
                        ...
                        bodyParas.pointSize = params.bodySize;
                        bodyParas.justification = params.bodyAlign;
                        ...
                        if (params.leadingIndex > 0) {
                            ...
                            bodyParas.leading = leadingValues[params.leadingIndex];
                        } else {
                            bodyParas.leading = Leading.AUTO;
                        }
        ...
        // Avtomatik yadda saxla
        if (params.autoSave) {
            try {
                doc.save();
                log("✓ Sənəd yadda saxlanıldı", "success");
            } catch (e) {
                log("! Yadda saxlama xətası: " + e.message, "warning");
            }
        }
    }
